    ...
    try {
        ...
        if (response.ok) {
            ...
            if (quantidadeDigitos >= 4 && data.total >= 12) {
                ...
                // Extrair e complementar os números até ter pelo menos 7
                const numerosUnicosArray = extrairNumerosUnicos(data.combinacoes);
                console.log("Números únicos extraídos (complementados se necessário):", numerosUnicosArray);
                
                // Calcular o total teórico de palpites - agora garantimos que temos pelo menos 7 números
                ...
                // Definir o limite máximo para o slider
...
// Função modificada para extrair números únicos e complementar caso necessário
function extrairNumerosUnicos(combinacoes) {
  const numerosUnicos = new Set();
  
  // Extrair números válidos das combinações
  for (const combinacao of combinacoes) {
    let i = 0;
    while (i < combinacao.length) {
      if (i + 1 < combinacao.length) {
        try {
          const numStr = combinacao.substring(i, i+2);
          const numero = parseInt(numStr);
          if (numero >= 1 && numero <= 31) { // Limitado aos números do Dia de Sorte (1-31)
            numerosUnicos.add(numero);
          }
        } catch (e) {}
      }
  ...
  let numerosUnicosArray = [...numerosUnicos];
  
  // Se não tivermos pelo menos 7 números, complementar com números adicionais
  if (numerosUnicosArray.length < 7) {
    // Criar lista de números possíveis que ainda não estão no conjunto
    const numerosFaltantes = [];
    for (let i = 1; i <= 31; i++) {
      if (!numerosUnicos.has(i)) {
        numerosFaltantes.push(i);
      }
    }
    
    // Adicionar números até completar pelo menos 7
    while (numerosUnicosArray.length < 7 && numerosFaltantes.length > 0) {
      // Escolher um número aleatório da lista de números faltantes
      const indiceAleatorio = Math.floor(Math.random() * numerosFaltantes.length);
      const numeroEscolhido = numerosFaltantes.splice(indiceAleatorio, 1)[0];
      numerosUnicos.add(numeroEscolhido);
    }
    
    // Atualizar o array
    numerosUnicosArray = [...numerosUnicos];
  }
  
  return numerosUnicosArray;
}
